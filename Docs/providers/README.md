...
**Every API account's ring follows DeepSeek's three modes** ([deepseek.md](deepseek.md#there-is-no-allowance-so-the-ring-has-no-denominator)): since top-up (a peak Pulse watched), a budget the reader typed, or the balance alone. `BalanceRing.applying` gives a live reading that is money and nothing else its one `.balance` window in the store, per account (`AppSettings.balanceBasis(for:)` / `balanceBudget(for:)`), with the watched peaks in `balance-baseline.json`, one per account and currency. A reading that already has limits of its own is left alone — those are the provider's figures. DeepSeek still makes its own ring in its service and keeps its own settings and file, from before the rule was everyone's; its settings pane rows (**Ring shows**, **Full tank**) are now every such account's.
